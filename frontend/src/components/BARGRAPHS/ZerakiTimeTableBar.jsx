...
const ZerakiTimetableBar = () => {

  const [primary_Count,setPCount] = useState(60);
  const [secondary_Count,setSCount] = useState(54);
  const [IGCSE_Count,setIGCount] = useState(77)
  ...
  useEffect(() => {
    try {
      fetch("http://localhost:8000/schools",{method:"GET"}).then(response =>{
        ...
        console.log(data);
        const primaryCount = data.filter(x => x.type === "Primary" && x.product === "Zeraki Timetable").length;
        const secondaryCount = data.filter(x => x.type === "Secondary" && x.product === "Zeraki Timetable").length;
        const IGCSECount = data.filter(x => x.type === "IGCSE" && x.product === "Zeraki Timetable").length;
