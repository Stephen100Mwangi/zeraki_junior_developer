  ...
  useEffect(() => {
    try {
      fetch("http://localhost:8000/schools",{method:"GET"}).then(response =>{
        ...
        console.log(data);
        const primaryCount = data.filter(x => x.type === "Primary" && x.product === "Zeraki Analytics" ).length;
        const secondaryCount = data.filter(x => x.type === "Secondary" && x.product === "Zeraki Analytics").length;
        const IGCSECount = data.filter(x => x.type === "IGCSE" && x.product === "Zeraki Analytics").length;
